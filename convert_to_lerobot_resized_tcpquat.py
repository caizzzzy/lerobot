...
# 输出目录
OUTPUT_ROOT = Path("data/lerobot_dataset_agilex0204_tcp_2")
...
def rpy_to_quaternion(roll, pitch, yaw):
    """
    Convert RPY (roll, pitch, yaw) to quaternion (qx, qy, qz, qw)
    """
    cy = math.cos(yaw * 0.5)
    sy = math.sin(yaw * 0.5)
...
def enforce_quaternion_continuity(quats):
    """
    quats: np.array (T,4) -> [qx,qy,qz,qw]
    返回连续序列，避免突然翻转
    """
...
def load_state_action(h5_file):
    with h5py.File(h5_file, 'r') as f:
        ...
        quats = np.array(quats, dtype=np.float32)
                ...
                # ================= 3️⃣ 四元数连续化 =================
        quats = enforce_quaternion_continuity(quats)

        # ================= 4️⃣ 四元数归一化 =================
        quats /= np.linalg.norm(quats, axis=1, keepdims=True)

        ee_pose = np.concatenate([xyz, quats], axis=1)  # (T,7)
