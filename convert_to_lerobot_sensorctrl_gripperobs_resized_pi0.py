...
# 输出目录
OUTPUT_ROOT = Path("data/lerobot_dataset_0405_pi0")
...
FPS = 30
TASK_DESCRIPTION = "Pick up the red bell pepper and place it into the box" 
...
def load_state_action(h5_file):
    with h5py.File(h5_file, 'r') as f:
        ...
        try:
            # 尝试使用 encoderDistance 路径 (这是你原始代码中的真实路径格式)
            gripper_state = f['gripper/encoderDistance/pikaGripper'][:]
            gripper_action = f['gripper/encoderDistance/pikaSensor'][:]
        except KeyError as e:
            # 如果路径不对，打印出 f['gripper'] 里面到底有什么，方便我们 debug
            print(f"\n[Debug] HDF5 键值错误: {e}")
            if 'gripper' in f:
                print(f"[Debug] 'gripper' 下第一层有: {list(f['gripper'].keys())}")
                if 'encoderDistance' in f['gripper']:
                    print(f"[Debug] 'gripper/encoderDistance' 下有: {list(f['gripper']['encoderDistance'].keys())}")
                if 'encoder' in f['gripper']:
                    print(f"[Debug] 'gripper/encoder' 下有: {list(f['gripper']['encoder'].keys())}")
            raise e # 继续抛出异常让外部 skip
            
        if gripper_state.ndim == 1: gripper_state = gripper_state[:, np.newaxis]
        if gripper_action.ndim == 1: gripper_action = gripper_action[:, np.newaxis]

        timestamps = f['timestamp'][:]
        
    state = np.concatenate([joint_pos, gripper_state], axis=1).astype(np.float32)
    action = np.concatenate([joint_pos, gripper_action], axis=1).astype(np.float32)
...
def convert():
    ...
    # ================= 1. 保存 Data Parquet =================
    if not dataset_rows:
        print("❌ 错误：所有 episode 都被跳过了，没有提取到任何有效数据！")
        print("请检查 load_state_action 函数中 HDF5 的读取路径是否正确。")
        return
